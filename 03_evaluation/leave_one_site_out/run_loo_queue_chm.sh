#!/usr/bin/env bash
# =====================================================================
# run_loo_queue_chm.sh  --  chew through LOO folds, one at a time
#
# WHAT IT DOES
...
#     nohup bash run_loo_queue_chm.sh > queue_$(hostname -s).out 2>&1 &
#
...
#
# WATCHING IT
#   bash run_loo_queue_chm.sh --status        prints progress and exits
#   tail -n 5 $PROJECT_ROOT/models/loo/runlogs/chm_site18_*.out
#
# IF A FOLD FAILS
...
#     rm -rf $PROJECT_ROOT/models/loo/queue_chm/chm_site7.failed
#
# STOPPING
#   pkill -f run_loo_queue          stops the runner after the current
#                                    fold; it does not kill a running fit
#   pkill -f loo_fold_chm           kills the fit itself
# =====================================================================
...
QUEUE=$PROJECT_ROOT/models/loo/queue_chm
...
# the data, so those folds are the expensive ones and must start first or
# they become a long tail at the end.
#   site:  18   17   14   15    20    8     13    10    12    9
#   rows:  23   27   407  484   1328  2374  2524  3206  3368  3802
#   site:  11   4    7    6     19    5
#   rows:  4116 4418 5966 6251  16402 70390
#
# Site 18 is already complete (pilot run, 10.88 h, all diagnostics passed),
# so it is listed but will be skipped once marked done.
CHM_SITES="18 17 14 15 20 8 13 10 12 9 11 4 7 6 19 5"
...
if [ ! -f loo_fold_chm.R ]; then
  echo "FATAL: loo_fold_chm.R not found in $SCRIPTS"; exit 1
fi
if [ ! -x "$RSCRIPT" ]; then
...
if [ "$FREE" -lt 64 ] && [ "${1:-}" != "--force" ]; then
  ...
  echo "Wait for load to fall, or override with:  bash run_loo_queue_chm.sh --force"
  exit 1
fi

echo "======================================================================"
echo "CHM LOO queue runner"
echo "host:         $HOST"
echo "started:      $(date)"
...
echo "each fold:    4 chains x 16 threads = 64 cores, 3000 iterations, adapt_delta 0.99"
echo "======================================================================"

for SITE in $CHM_SITES; do
  TASK="chm_site${SITE}"
  ...
  nice -n 10 "$RSCRIPT" loo_fold_chm.R --site "$SITE" \
